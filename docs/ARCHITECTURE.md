...
5. **Aislamiento entre espacios de trabajo**: lo impone el servidor por membresía (`requireWorkspaceMember`). Las políticas RLS que filtran por membresía —espacios, membresías y tableros— se apoyan en una tabla dedicada de membresías y funciones SQL `SECURITY DEFINER` para evitar recursión (ADR-012/009); las de tarjetas, columnas y categorías filtran solo por organización. Aquí ponía «Aislamiento Multi-tenant»: la organización es una sola, por decisión (ADR-020).
...
**Fecha:** 2026-04-13
**Estado:** ⚠️ **Superado por ADR-027 (25-ago-2026): no queda digest que contextualizar.** Se conserva porque explica por qué el botón de correo era contextual, no para describir lo que hoy hace la aplicación — hoy ese botón no existe.
...
### ADR-027: La nave no manda correo
**Fecha:** 2026-08-25
**Estado:** Aceptado
**Contexto:** Decisión de Ibai: **«Cero mails. De todos modos, nunca los leo».** La nave tenía dos digests —el de administrador y el personal—, un reloj horario en GitHub Actions, tres rutas de disparo, un envoltorio de envío sobre Resend y una preferencia por usuario para elegir la hora. Nadie los leía. Según el gerente financiero del Orquestador, `digest-cron.yml` era 296 de las 356 corridas del repo entre el 12 y el 25 de agosto — el 83% del coste vivo de la nave.
**Decisión:** Retirar **todo** camino por el que la aplicación pueda mandar un correo: el workflow, `server/routes/digestRoute.js`, `server/services/digest/`, `server/utils/mailer.js`, `server/utils/digestLogging.js`, `PATCH /api/auth/me/preferences`, la preferencia de hora en la GUI, el botón de digest de la toolbar y la dependencia de Resend. Con ellos se van sus guardianes y pruebas, que pasarían a vigilar código inexistente.
**Consecuencias:** La notificación **in-app** sobrevive intacta —asignar una tarjeta sigue sonando por la campana, y el contrato lo declara—; lo que se pierde es el resumen diario, no el aviso. **El correo era la única salida de datos personales del sistema hacia fuera**, así que retirarlo reduce superficie. `RESEND_API_KEY`, `SMTP_FROM` y `DIGEST_CRON_SECRET` dejan de leerse: si siguen puestas, retirarlas es limpieza pendiente y no un requisito de arranque. **Queda esquema sin lector** —`users.digest_hour`, `users.digest_enabled` y la tabla `digest_logs`— y se deja en pie a propósito: `digest_logs` es historia de envíos ya ocurridos y borrarla es una migración con su propio ciclo. Queda declarado en `docs/schema/supabase-schema.sql` para que no parezca un olvido; **decidir si se retira es de Ibai, no del que ejecutó esto.**
