...
#
# `version` y `tests` son del commit medido. **`tablas_rls` y `policies_rls` NO:
...
#   DATABASE_URL        para las dos cifras de RLS
#
# Costuras para el sello (sin red y sin base):
#   CIFRAS_TABLAS_RLS / CIFRAS_POLICIES_RLS   saltan la consulta a la base
...
if [ -n "${CIFRAS_TABLAS_RLS+x}" ] || [ -n "${CIFRAS_POLICIES_RLS+x}" ]; then
  tablas_rls="${CIFRAS_TABLAS_RLS:-}"
  policies_rls="${CIFRAS_POLICIES_RLS:-}"
else
  conn="${DATABASE_URL:-}"
  [ -n "$conn" ] || falla 2 "falta DATABASE_URL: sin base no hay cifras de RLS, y no se publica un JSON incompleto"
  command -v psql >/dev/null 2>&1 || falla 2 "falta psql en el corredor"
  tablas_rls="$(psql "$conn" -t -A -v ON_ERROR_STOP=1 -c \
    "SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
      WHERE n.nspname = 'public' AND c.relkind IN ('r','p') AND c.relrowsecurity;")" \
    || falla 2 "la consulta de tablas con RLS falló"
  policies_rls="$(psql "$conn" -t -A -v ON_ERROR_STOP=1 -c \
    "SELECT count(*) FROM pg_policies WHERE schemaname = 'public';")" \
    || falla 2 "la consulta de políticas falló"
fi
...
echo "publicar-cifras: publicado en $RAMA — version $version · $tests tests · $tablas_rls tablas con RLS · $policies_rls políticas."
