...
function crearHostMonitor({
  ...
  // (host+ruta) → { primeraVez, vistas }
  const acumulado = new Map();

  return function hostMonitor(req, res, next) {
    ...
    avisar(
      `[B-03 monitor] acceso fuera del dominio esperado: host=${host} path=${req.path} ` +
...
module.exports = { crearHostMonitor, VENTANA_MS };
